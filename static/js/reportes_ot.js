...
(function () {
  ...
  async function loadOTs() {
    ...
    if (body) body.innerHTML = '<tr><td colspan="10" class="text-muted">Cargando…</td></tr>';
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        if (body) {
          body.innerHTML = `<tr><td colspan="10" class="text-danger">Error: ${data.message || res.status}</td></tr>`;
        }
        return;
      }
      ...
      if (items.length === 0) {
        if (body) {
          body.innerHTML = '<tr><td colspan="10" class="text-muted">Sin registros en el rango / filtros.</td></tr>';
        }
        return;
      }
      ...
      if (body) {
        body.innerHTML = '<tr><td colspan="10" class="text-danger">Error de red</td></tr>';
      }
    }
  }
